...
pair<int,int> index (int n) {

  if ((n/2)%2)         //si el truncamiento de su mitad es impar
    return pair<int,int> (n-1,(n+1)%(num_filosofos*2));
  else
    return pair<int,int> ((n+1)%(num_filosofos*2),n-1);
}
...
void funcion_tenedores( int id )
{
  ...
  while ( true )
    {
      MPI_Recv ( &valor, 1, MPI_INT, index( id ).first , 0, MPI_COMM_WORLD, &estado );
      ...
      MPI_Recv ( &valor, 1, MPI_INT, index( id ).second , 0, MPI_COMM_WORLD, &estado );
      id_filosofo = estado.MPI_SOURCE;// ...... guardar en 'id_filosofo' el id. del emisor (completar)
      cout <<"Ten. " <<id <<" ha sido cogido por filo. " <<id_filosofo <<endl;
